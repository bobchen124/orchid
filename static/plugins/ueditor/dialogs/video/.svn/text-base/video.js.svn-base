...
(function(){
    ...
    function insertSingle(){
        ...
        if(!url) return false;
        if(!video.url) {
        	$G("preview").innerHTML ="输入的视频地址有误，请输入www.uubridge.net网站上的视频！";
        	return false;
        }
    ...
    function getVideoUrl(url){
    	var getUrl="/pages_l4/js/plugin/ueditor/dialogs/video/watch_get_url_processor.jsp?lang=zh_cn&location=cn";
        ajax.request(getUrl,{
        	videoUrl:url,
            onsuccess:function(xhr){
                ...
                video.url=info.echo.url;
                var videoPath=video.url;
                //alert(videoPath)
                if(!video.url){
                	$G("preview").innerHTML = "输入的视频地址有误，请输入www.uubridge.net网站上的视频！";
                	return false;
                }
                var num=Math.random()*100000000000000000;
                var strs='<p><div id="mediaplayer_wrapper'+num+'" style="position: static; width: 420px; height: 280px; "><p id="jwplayer'+num+'"></div><p>';
                $G("preview").innerHTML =strs;
                var previewElement = document.getElementById('mediaplayer_wrapper'+num);
                var s = document.createElement('script');
                s.setAttribute('type','text/javascript');
                s.setAttribute('src','http://www.uubridge.net/pages/common/js/plugin/jwplayer/initjw.js');
                s.setAttribute('path',videoPath);
                s.setAttribute('jwid','jwplayer'+num);
                s.setAttribute('width',420);
                s.setAttribute('height',280);
                previewElement.appendChild(s);
            }
        });
    }
    ...
    function createPreviewVideo(url){
        ...
        var matches1 = url.match(/www\.uubridge\.net\/\w+\/\w+\/v\/(\d+)\/\d+\.html$/);
        var matches2 = url.match(/www\.uubridge\.net\/pages_l4\/homepage\/homepage_video_watch\.jsp\?(\w+=\w+)&(\w+=\w+)&(\w+=\w+)(?:&(\w+=\w+))?$/);
        if(matches1){
        	getVideoUrl(matches1[1]);
        }else if(matches2){
        	for(var i=1;i<matches2.length;i++){
        		var matchId=matches2[i]==undefined?null:matches2[i].match(/^videoId=(\d+)$/);
        		if(matchId){
        			getVideoUrl(matchId[1]);
        			return;
        		}
        	}
        	video.url="";
